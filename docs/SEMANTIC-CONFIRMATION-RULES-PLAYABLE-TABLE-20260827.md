...
当前累计合同已在 `DEC-20260827-022` 中完成唯一绑定，并作为 `DEC-20260827-020` 的规则后继；旧 `DEC-20260825-008` 的亮牌规则由第 4 条吸收，Ready、掉线和退出规则取得当前权威。后续独立的“公开座位 AI 交流规则包”也已由 `DEC-20260827-023` 确认；当前不再存在待确认的产品规则包，下一阶段是受影响实现与宿主证据重验。
