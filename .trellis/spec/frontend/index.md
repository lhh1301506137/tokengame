...
- 当前产品是一条本地纵向切片：宿主中立权威内核 + 宿主侧协调器 + 浏览器牌桌，2–4 名隔离玩家可完成「建房/邀请码加入 → 逐席公开范围确认 → Ready → 连续多手无限注德州 → 公开聊天 → 座位 AI 公开发言或沉默 → 掉线恢复/暂离/离桌 → 本地隐藏」。仍不是生产牌室：没有账号、没有大厅、没有远程部署。
...
- Node.js 代码使用 CommonJS `.cjs`；浏览器模块使用 `.mjs`/`type="module"`，旧探针栈的 `web/app.js` 保持普通 `.js`。
