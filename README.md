...
npm run beta      # 朋友私人房内测：起协调器 + 生成模型令牌 + 打印加入说明
npm run web       # 只起浏览器牌桌（模型命令口关着）
```

**要让各人的宿主 AI 在座位旁说话，用 `npm run beta`。** 它比 `npm run web` 多做一件 B6 之后
必需的事：生成模型命令口的令牌并接上协调器。`npm run web` 不生成它，所以那条路是关着的
（`/api/health` 会显示 `model_command_route: disabled`）——牌桌照常能玩，只是没有任何一席的
宿主 AI 能发言。

`npm run beta` 打印牌桌地址、加入步骤，以及模型令牌所在的文件路径。令牌**不打印在终端里**：
人要把它填进宿主的 MCP 注册项，而那一步在编辑器里做，打印出来只会让它进入终端回滚、
`> log.txt` 与截屏。

建房的人在页面上创建房间、拿到邀请码转给朋友；每位朋友在**独立浏览器上下文**里打开同一地址
并用邀请码加入。不要交换带令牌的 URL。
...
| 宿主插件（MCP） | 什么都不持有，只有模型令牌 | 凭据、句柄、会话令牌 |

插件那一侧要的两个环境变量是 `TOKENGAME_MODEL_TOKEN`（必填）与 `TOKENGAME_TABLE_ORIGIN`
（用约定端口 7802 时不用填）。`TOKENGAME_COMMAND_ORIGIN` 是协调器连内核用的，不是插件的配置。
