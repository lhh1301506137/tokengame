...
<a id="b33-poker-maturity-and-refill"></a>
## 2026-09-05 — B33：成熟德扑规则对标与破产补筹闭环

状态：本地实现、浏览器闭环、最终串行完整门禁及远端同一候选四作业CI均已完成；双好友真人验收未执行。

### 对标判断与产品边界

用户要求在真人验收前先与成熟开源实现比较，确认普通德扑玩法没有会阻断闭环的缺口。Primary在仓库外
临时检出PokerKit提交`54571ddda38b7da9b8c527d54c16c788ac14dac7`和`@hivetech/poker-engine`
提交`f23776d435b66c2d23ff2c0fe95c1e69448a3865`，核对许可证、规则实现与测试；后者自身测试实际24/24。
另以Poker TDA和WSOP公开规则核对短额all-in与加注语义。没有复制外部源码、没有加入依赖，临时检出
不在仓库。结论是保留现有小型规则内核：更换整仓不会解决TokenGame的权威、私有投影、AI、迟到归约和
Connector问题，反而需要重接已验证边界。基础覆盖和明确后置项见`docs/POKER-MVP-COVERAGE.md`。

### 实现与首次闭环

规则层将不完整大盲仍视为名义大盲，跟注金额按实际对手投入计算；没有能够回应的对手时不再暴露bet、
raise或all-in，且无人需要/能够行动时确定性结束下注轮。新增六条成熟规则反例、全部2,598,960种五张牌
类别分布穷举，以及1,000条固定种子合法行动序列的终止与筹码守恒检查。

用户以`DEC-20260905-001`确认好友现金桌使用不可兑现测试筹码：破产席手后SIT_OUT，仅手间由真人固定
补回房间起始200，不能自定义或自动补，补完不自动Ready，模型无权调用。命令、席位授权、投影、Web按钮、
错误码和好友验收清单一起落地。双隔离Chromium实际完成建房、加入、确认、Ready、all-in/call、破产、
补筹与另按Ready，11项通过、控制台0错误、6033ms；两张关键截图由Primary目检。

在第一次完整绿灯后的收尾审读中又发现：零筹码SIT_OUT席若走核心`seat.recover`，旧恢复逻辑会统一
改成`SEATED`；它随后既被Ready要求先补筹，又因不是SIT_OUT而不能补筹。浏览器保留会话的短断恢复
通常不走这条分支，但宿主恢复合同会。实现改为零筹码恢复继续保持SIT_OUT，新增领域用例与
`REFILL-18`反向变异；相邻78/78、补筹18/18通过。

### 验证过程与未关闭边界

聚焦Node在最终两条名单门禁反例加入前曾65/65，加入后相关三个文件49/49。新规则变异7/7、补筹变异
17/17全杀。首轮完整Node注册1488项，1487通过、1项平台条件跳过、0失败，耗时92203.1273ms。
同一代码上的首轮完整`npm run gate`真实exit1：717条中712杀掉、3存活、2未评估。三项存活是跨手
名单/门禁测试没有构造异常零栈READY/ACTIVE状态；两项未评估是新增命令和筹码显示使旧变异锚点过期。
补测试与更新精确锚点后，相关Node49/49；一次把三个会临时改写共享源码的变异套件并发运行造成F4假存活，
该编排错误已停止，F4随后串行14/14全杀。定向修补后的第二次完整串行`npm run gate`约601秒、
真实exit0：Node1490/1490、变异717/717全杀。加入破产恢复修补后，最终字节第三次完整串行门禁
约697秒、真实exit0：Node1491/1491、变异718/718全杀，0存活、0未评估。它重新覆盖F1 15/15、
F4 14/14、Web 16/16、成熟规则7/7和补筹18/18，并由门禁自己顺序恢复每个变异；并发结果没有被
用作最终通过依据。
机器可读摘要见`.trellis/tasks/08-26-public-ai-table-talk/research/b33-poker-maturity-refill-20260905.json`。

本批原生模型、原生queue、公网隧道调用均为0。真实两机、两真人、双Codex、双方同手公开AI气泡、
十手与签字仍为`not_run`；B33自动化全绿只把候选交到真人验收门口。用户已以
`DEC-20260905-002`授权提交、非强制推送和CI核对；实现提交为
`610a8a175ec7938a894227bd7853f6259ab91659`。授权本身不等于远端成功，需先核对实际远端SHA及同一候选CI，
随后两名玩家再按`docs/REMOTE-FRIEND-MVP.md`执行。

### 后续发布回执（不改写上面的本地验证过程）

用户以`DEC-20260905-002`明确授权本批commit+push。实现提交
`610a8a175ec7938a894227bd7853f6259ab91659`与就绪记录提交
`97b4c946b2e572e5460babddbb8857abdbb0bbdb`已非强制推送；`git ls-remote`确认远端main精确指向
`97b4c946b2e572e5460babddbb8857abdbb0bbdb`。对应GitHub Actions run
[`33909572989`](https://github.com/lhh1301506137/tokengame/actions/runs/33909572989)的headSha相同，四个作业全部success：
Windows Node 22为1491/1491（98199.9287ms），Windows Node 24为1491/1491（97880.7915ms），
Ubuntu Node 22为1483/1483（74081.70325ms），Ubuntu Node 24为1483/1483（73414.873414ms）；
四项的fail/cancelled/skipped/todo均为0。

本次远端只运行`npm test`，没有在GitHub重跑718条变异或Chromium补筹验收；二者仍引用同一实现字节的
本地最终门禁和浏览器记录。发布没有启动隧道、模型或牌局。真实两设备、两真人、两个独立Codex、
双方同手公开AI气泡、至少十手、断线恢复和签字仍全部是`not_run`，所以MVP与父任务保持开放。
